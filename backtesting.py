...
def executar_backtest_modelo(modelo_file, engine, dados_dir):
    """
    Executa backtest para um modelo específico.
    
    Args:
        modelo_file (str): Caminho do arquivo do modelo
...
def gerar_relatorio_final(resultados):
    ...
    logger.info("💾 Resultados salvos em: resultados_backtest.csv")
...
def executar_backtesting():
    ...
    for modelo_file in modelos_disponiveis:
        resultado = executar_backtest_modelo(modelo_file, engine, dados_dir)
        if resultado:
...
class EstrategiaML(Strategy):
    ...
    def init(self):
        ...
        # RSI
        self.rsi = self.I(lambda x: ta.rsi(pd.Series(x), length=14), self.data.Close)
        
        # MACD
        macd_data = self.I(lambda x: ta.macd(pd.Series(x)), self.data.Close)
        self.macd = self.I(lambda x: x['MACD_12_26_9'] if isinstance(x, pd.DataFrame) and 'MACD_12_26_9' in x.columns else pd.Series([np.nan]*len(x)), macd_data)
        self.macd_signal = self.I(lambda x: x['MACDs_12_26_9'] if isinstance(x, pd.DataFrame) and 'MACDs_12_26_9' in x.columns else pd.Series([np.nan]*len(x)), macd_data)
        self.macd_hist = self.I(lambda x: x['MACDh_12_26_9'] if isinstance(x, pd.DataFrame) and 'MACDh_12_26_9' in x.columns else pd.Series([np.nan]*len(x)), macd_data)
        
        # Bollinger Bands
        bb_data = self.I(lambda x: ta.bbands(pd.Series(x)), self.data.Close)
        self.bb_upper = self.I(lambda x: x['BBU_20_2.0'] if isinstance(x, pd.DataFrame) and 'BBU_20_2.0' in x.columns else pd.Series([np.nan]*len(x)), bb_data)
        self.bb_middle = self.I(lambda x: x['BBM_20_2.0'] if isinstance(x, pd.DataFrame) and 'BBM_20_2.0' in x.columns else pd.Series([np.nan]*len(x)), bb_data)
        self.bb_lower = self.I(lambda x: x['BBL_20_2.0'] if isinstance(x, pd.DataFrame) and 'BBL_20_2.0' in x.columns else pd.Series([np.nan]*len(x)), bb_data)
        
        # Stochastic
        stoch_data = self.I(lambda h, l, c: ta.stoch(pd.Series(h), pd.Series(l), pd.Series(c)), 
                           self.data.High, self.data.Low, self.data.Close)
        self.stoch_k = self.I(lambda x: x['STOCHk_14_3_3'] if isinstance(x, pd.DataFrame) and 'STOCHk_14_3_3' in x.columns else pd.Series([np.nan]*len(x)), stoch_data)
        self.stoch_d = self.I(lambda x: x['STOCHd_14_3_3'] if isinstance(x, pd.DataFrame) and 'STOCHd_14_3_3' in x.columns else pd.Series([np.nan]*len(x)), stoch_data)
        
        # ATR
        self.atr = self.I(lambda h, l, c: ta.atr(pd.Series(h), pd.Series(l), pd.Series(c)), 
                         self.data.High, self.data.Low, self.data.Close)
    ...
    def next(self):
        ...
        try:
            ...
            if confianca >= self.confianca_minima:
                if predicao == 1 and not self.position:  # Sinal de alta forte
                    self.buy(sl=self.data.Close[-1] * 0.92, tp=self.data.Close[-1] * 1.15)
                elif predicao == 2 and self.position:  # Sinal de baixa forte
                    self.position.close()
