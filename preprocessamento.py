...
def processar_dados_brutos():
    ...
    if not os.path.exists('dados_processados'):
        ...
        logger.info("Diretório 'dados_processados' criado!")
    
    # Obter lista de arquivos na pasta dados_brutos
    if not os.path.exists('dados_brutos'):
        logger.error("Pasta 'dados_brutos' não encontrada!")
    ...
    if not arquivos_brutos:
        logger.error("Nenhum arquivo CSV encontrado na pasta 'dados_brutos'!")
        return {'sucessos': 0, 'falhas': 0, 'total': 0}
