...
def plotar_feature_importance(modelo, feature_names, nome_modelo):
    ...
    try:
        ...
        nome_arquivo = os.path.join(results_dir, f'feature_importance_{nome_modelo}.png')
...
def processar_arquivo_modelo(caminho_arquivo, config):
    ...
    try:
        ...
        caminho_modelo = os.path.join(models_dir, f'modelo_{nome_base}.joblib')
        joblib.dump({
            'modelo': modelo,
            'feature_names': feature_names,
            'metricas': metricas,
...
def treinar_modelos(config):
    ...
    if not os.path.exists(dados_processados_dir):
        logger.error(f"Pasta '{dados_processados_dir}' não encontrada!")
    ...
    arquivos_processados = [
        f for f in os.listdir(dados_processados_dir) 
    ...
    for i, arquivo in enumerate(arquivos_principais, 1):
        ...
        caminho_arquivo = os.path.join(dados_processados_dir, arquivo)
...
def main():
    ...
    if estatisticas['sucessos'] > 0:
        ...
        if os.path.exists(models_dir):
            modelos = [f for f in os.listdir(models_dir) if f.startswith('modelo_') and f.endswith('.joblib')]
